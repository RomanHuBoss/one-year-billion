const HELP_TEXT = {
  ...
  hero: {
    ...
    safeWhen: 'Для локальной работы нормальны статусы «Безопасный локальный режим» или «Безопасный testnet/local режим» и blocked по live-gate. Для реальной торговли нужен только полный PASS.',
...
const REASON_HELP = {
  ...
  bybit_private_api_auth_failed: 'Public API работает, но private API не принял ключ или подпись. Если retCode=10002 — это чаще всего рассинхрон времени/recv_window, а не плохой ключ.',
  bybit_timestamp_window_error_10002: 'Bybit отклонил private request по timestamp/recv_window. Backend синхронизирует подпись с Bybit server time; при повторе включите синхронизацию часов Windows.',
