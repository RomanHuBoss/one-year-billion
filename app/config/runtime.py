...
def build_runtime_config(raw: dict[str, Any] | None = None) -> RuntimeConfig:
    ...
    risk = RiskConfig(
        ...
        max_daily_loss_pct=float(risk_yaml.get('max_daily_loss_pct', 0.03)),
        max_weekly_loss_pct=float(risk_yaml.get('max_weekly_loss_pct', 0.06)),
        max_portfolio_abs_notional_usdt=(float(risk_yaml['max_portfolio_abs_notional_usdt']) if risk_yaml.get('max_portfolio_abs_notional_usdt') is not None else None),
        max_beta_adjusted_exposure_usdt=(float(risk_yaml['max_beta_adjusted_exposure_usdt']) if risk_yaml.get('max_beta_adjusted_exposure_usdt') is not None else None),
