...
| CF-08/CF-16: frontend не source of truth | `frontend/js/status_contract.js`, `frontend/js/app.js`, `/api/state/overview`, `latest_symbol_status` | API contract + manual E2E; `tests/test_live_gates.py`, `tests/test_db_status_contract_static.py` |
...
| Risk approval всегда требует RBAC/idempotency | `app/api/routes/risk.py`, `app/execution/idempotency.py` | `tests/test_live_risk_approval_gates.py` |
