...
python scripts/record_go_no_go_evidence.py --type CI --status PASS --metrics-json '{"tests":139}'
...
Локальная проверка последней редакции: `python main.py validate` — PASS, `139 passed`. Testnet/live preflight в песочнице корректно возвращают `blocked`, потому что нет внешней PostgreSQL, Bybit API keys и Go/No-Go evidence.
