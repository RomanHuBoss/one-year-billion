...
Локальная проверка последней редакции: `python main.py validate` — PASS, `108 passed`. Testnet/live preflight в песочнице корректно возвращают `blocked`, потому что нет внешней PostgreSQL, Bybit API keys и Go/No-Go evidence.
